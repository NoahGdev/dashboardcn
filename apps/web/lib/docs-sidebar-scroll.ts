...
export const DOCS_SIDEBAR_SCROLL_RESTORE_SCRIPT = `(function () {
  ...
  var stored = null
  try { stored = JSON.parse(sessionStorage.getItem("${DOCS_SIDEBAR_SCROLL_STORAGE_KEY}")) } catch (e) {}
  function getActiveItem(container) {
    var items = container.querySelectorAll('[data-active="true"]')
    var active = null, activePathLength = -1, activeDistance = Infinity
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (pathLength > activePathLength || (pathLength === activePathLength && distance < activeDistance)) {
        active = items[i]; activePathLength = pathLength; activeDistance = distance
      }
    }
    return active
  }
  function restoreScroll() {
    ...
    if (stored && stored.pathname === location.pathname) { container.scrollTop = stored.scrollTop; return }
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", function () { restoreScroll(); observer.disconnect() }, { once: true })
  } else {
    observer.disconnect()
  }
})()`
